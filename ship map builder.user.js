...
// ============================================================
// CONSTANTS — source: utils/constants.js
// ============================================================
// === CONSTANTS ===
// ============================================================
...
// ============================================================
// HELPERS — source: utils/helpers.js
// ============================================================
// === HELPERS ===
// ============================================================
// DEBOUNCE UTILITY
...
// ============================================================
// LIFECYCLE — source: utils/lifecycle.js
// ============================================================
// === LIFECYCLE ===
// ============================================================
...
// ============================================================
// MATCH INDEX — source: utils/match-index.js
// ============================================================
// === MATCH INDEX ===
// ============================================================
...
// ============================================================
// SITE SETTINGS — source: utils/site-settings.js
// ============================================================
// === SITE SETTINGS ===
// ============================================================
// SITE SETTINGS
...
const State = {
    ...
    dashboardMode: false,
    // ── Shift Snapshot (cancelled detection) ──
    _shiftSnapshotLabel: null,
    _shiftSnapshotVrIds: new Map(),  // vrId → { route, carrier, sdt, status }
    _shiftCancelledVrIds: new Map(), // vrId → { route, carrier, sdt }
    // ── Focus Mode ──
...
const SSP = {
    ...
    async fetchData() {
        ...
        try {
            ...
            silentCatch('shiftSnapshot', () => {
                const shift = getCurrentShift();
                if (State._shiftSnapshotLabel !== shift.label) {
                    // New shift — reset snapshot
                    State._shiftSnapshotLabel = shift.label;
                    State._shiftSnapshotVrIds = new Map();
                    State._shiftCancelledVrIds = new Map();
                }
                // Add all current VR IDs to snapshot (first seen wins)
                for (const l of loads) {
                    if (!l.vrId) continue;
                    const vr = l.vrId.toUpperCase();
                    if (!State._shiftSnapshotVrIds.has(vr)) {
                        State._shiftSnapshotVrIds.set(vr, { route: l.route, carrier: l.carrier || '', sdt: l.sdt, status: l.status });
                    }
                }
                // Detect cancelled: was in snapshot, now missing AND not departed/completed
                const currentVrIds = new Set(loads.filter(l => l.vrId).map(l => l.vrId.toUpperCase()));
                for (const [vr, info] of State._shiftSnapshotVrIds) {
                    if (currentVrIds.has(vr)) {
                        // Still present — check if it became CANCELLED explicitly
                        const cur = loads.find(l => l.vrId && l.vrId.toUpperCase() === vr);
                        if (cur && cur.status === 'CANCELLED') {
                            State._shiftCancelledVrIds.set(vr, { route: info.route, carrier: info.carrier, sdt: info.sdt });
                        } else {
                            State._shiftCancelledVrIds.delete(vr); // back alive
                        }
                    } else if (info.status !== 'DEPARTED' && info.status !== 'COMPLETED') {
                        // Gone and wasn't departed — cancelled
                        State._shiftCancelledVrIds.set(vr, { route: info.route, carrier: info.carrier, sdt: info.sdt });
                    }
                }
            });
...
// ============================================================
// DOCKMASTER API — source: api/dockmaster.js
// ============================================================
// === DOCKMASTER API ===
    // ============================================================
...
const Summary = {
    build() {
        ...
        return { shift, shiftProgress, remH, remM, now, loads:{total:loads.length,departed,loading,finished,attached,ready,readyForLoad,scheduled,cancelled,inProgress,carriers,activeDocks:activeDocks.size,swapFiltered:State.swapFilteredCount,cancelledDetected:State._shiftCancelledVrIds.size,cancelledList:[...State._shiftCancelledVrIds.entries()].map(([vr,info])=>({vrId:vr,...info}))}, nonInv:this._buildNonInvSummary(shift), yms:{total:ymsTotal,empty:ymsEmpty,full:ymsFull,unavail:ymsUnavail,occupied:ymsOccupied,totalLocs:State.ymsLocations.length,utilization:ymsUtilization,report:ymsReport}, vista:{total:vc.length,stacked:vStacked,staged:vStaged,loaded:vLoaded,totalPkgs:vTotalPkgs,congestion:congestionCounts,topCongested,topDwell,avgDwell,maxDwell,totalLocs:Object.keys(State.vistaLocMap).length} };
    },
    _buildNonInvSummary(shift) {
        const shiftStart = shift.startDate.getTime();
        const shiftEnd = shift.endDate.getTime();
        // FMC NonInv tours — filter to current shift by planned arrival or actual arrival
        const fmcRaw = FMC.getNonInvTours();
        const fmcAll = fmcRaw.filter(t => {
            const time = t.actualYardArrival || t.plannedYardArrival || t.plannedDockArrival || 0;
            return time >= shiftStart && time <= shiftEnd;
        });
        const fmcCompleted = fmcAll.filter(t => t.hasDeparted || t.tourStatus === 'DEPARTED').length;
        const fmcAtYard = fmcAll.filter(t => t.hasArrived && !t.hasDeparted).length;
        const fmcPlanned = fmcAll.filter(t => !t.hasArrived && !t.hasDeparted).length;
        // Breakdown by shipper/type
        const byType = {};
        for (const t of fmcAll) {
            const key = t.shippers.join(', ') || t.businessTypesList.join(', ') || t.shipperCategory || 'Unknown';
            if (!byType[key]) byType[key] = { total: 0, completed: 0, atYard: 0, planned: 0 };
            byType[key].total++;
            if (t.hasDeparted || t.tourStatus === 'DEPARTED') byType[key].completed++;
            else if (t.hasArrived && !t.hasDeparted) byType[key].atYard++;
            else byType[key].planned++;
        }
        // Dockmaster NonInv — filter to current shift by scheduled start
        const dmRaw = Dockmaster.getNonInv();
        const dmAll = dmRaw.filter(a => {
            const time = a.checkInStart || a.schedStart || 0;
            return time >= shiftStart && time <= shiftEnd;
        });
        const dmCompleted = dmAll.filter(a => a.status === 'COMPLETED').length;
        const dmActive = dmAll.filter(a => a.status === 'ARRIVED' || a.status === 'RECEIVING').length;
        const dmScheduled = dmAll.filter(a => a.status === 'SCHEDULED').length;
        return { fmcTotal: fmcAll.length, fmcCompleted, fmcAtYard, fmcPlanned, dmTotal: dmAll.length, dmCompleted, dmActive, dmScheduled, byType, total: fmcAll.length + dmAll.length, totalCompleted: fmcCompleted + dmCompleted };
    },
    ...
    render(d) {
        ...
        const cancelledRows = d.loads.cancelledList.length ? d.loads.cancelledList.map(c => `<tr><td style="font-family:monospace;color:#ff5252">${c.route}</td><td style="color:#e0e0e0;font-size:10px">${c.vrId}</td><td style="color:#78909C;font-size:10px">${c.carrier||'—'}</td></tr>`).join('') : '';

        const niTypeRows = Object.entries(d.nonInv.byType).sort((a,b)=>b[1].total-a[1].total).map(([name,s])=>`<tr><td style="font-weight:bold;color:#e0e0e0;font-size:10px">${name}</td><td class="val" style="color:#69f0ae">${s.completed}</td><td class="val" style="color:#4fc3f7">${s.atYard}</td><td class="val" style="color:#ffd600">${s.planned}</td><td class="val" style="color:#ff9900">${s.total}</td></tr>`).join('');
    ...
    toClipboard(d) {
        ...
        if (d.loads.cancelledDetected) { t += ` ❌ Cancelled: ${d.loads.cancelledDetected}\n`; for (const c of d.loads.cancelledList) t += `   ${c.route} | ${c.vrId} | ${c.carrier||'—'}\n`; }
        t += `\n🔀 NONINV: ${d.nonInv.total} awizowane / ${d.nonInv.totalCompleted} rozładowane\n FMC: ${d.nonInv.fmcTotal} (✅${d.nonInv.fmcCompleted} 🏗${d.nonInv.fmcAtYard} 📅${d.nonInv.fmcPlanned})\n DM: ${d.nonInv.dmTotal} (✅${d.nonInv.dmCompleted} 🏗${d.nonInv.dmActive} 📅${d.nonInv.dmScheduled})\n`;
        for (const [name, s] of Object.entries(d.nonInv.byType)) t += `   ${name}: ${s.total} (✅${s.completed} 🏗${s.atYard} 📅${s.planned})\n`;
        t += '\n';
...
${eyeBtn}
<span class="fmc-tour-carrier">${l.carrier || ''}</span>
